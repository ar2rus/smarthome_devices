...
p.add_argument('--without-editor',action='store_true',help='Build ESP without the optional /edit handler')
...
def run(label,cmd):
 ...
 return r.returncode
editor_flags=['--build-property','compiler.cpp.extra_flags=-DSMARTHOME_HAS_FILE_EDITOR=0'] if a.without_editor else []
esp=run('esp',[a.arduino_cli,'compile','--config-file',str(config),'--fqbn',a.fqbn,'--warnings','all','--library',str(a.library),'--build-path',str(out/'esp-build'),'--jobs','4',*editor_flags,str(root/'bridge/src/SmarthomeBridge')])
avr=run('avr',[str(a.avr_bin/'avr-gcc'),'-mmcu=atmega8a','-O2','-std=gnu99','-DF_CPU=16000000UL','-funsigned-char','-funsigned-bitfields','-fpack-struct','-fshort-enums','-Wall','-I'+str(root/'_lib'),'-I'+str(root/'bridge/src/Bridge'),'-I'+str(root/'bridge/src/Bridge/clunet'),str(root/'bridge/src/Bridge/Bridge.c'),str(root/'_lib/clunet/clunet.c'),str(root/'_lib/clunet/clunet_buffered.c'),'-o',str(out/'Bridge.elf')])
for line in (out/'esp-build.log').read_text().splitlines():
 if 'used ' in line and not 'warning:' in line:print(line)
if not avr:
 print(subprocess.check_output([str(a.avr_bin/'avr-size'),'-C','--mcu=atmega8a',str(out/'Bridge.elf')],text=True))
